...
<%
				...
				totalRecord = uMgr.getTotalTrainer(keyField, keyWord);
	    ...
	    delButtons.forEach(function(button){
	    	button.addEventListener('click', function(){
	    		...
	    		if(isEditing === 'true'){
	    			...
	    			var form = document.getElementById('TrainerDelForm');
	            ...
	            if(isEditing === 'true') {
	                ...
	                [tr].forEach(function(row) {
	                    Array.from(row.cells).forEach(function(td, index) {
	                        if(row === tr && (index == 1 || index == 3   || index == 7 )) {
	                ...
	                [nextTr].forEach(function(row) {
	                    Array.from(row.cells).forEach(function(td, index) {
	                        if(row === nextTr && (index == 1  || index == 5 || index == 6 )) { 
	                            var input = td.querySelector('input');
	                            if(input && index !== 0) {
	                               
	                                switch(index) {
	                                    case 1: nextTr1 = input.value; break;
	                                    case 5: nextTr5 = input.value; break;
	                                    case 6: nextTr6 = input.value; break;
	                                }
	    	            ...
	    	            var form = document.getElementById('TrainerEditForm');
		    	         ...
		    	         form.elements['nextTr6'].value = nextTr6;
	            	...
	            	[tr, nextTr].forEach(function(row) {
	            	    Array.from(row.cells).forEach(function(td, index) {
	            	        if(index != 0) { 
	            	            if((row === tr && (index == 1 || index == 3  || index == 7 )) ||
	            	               (row === nextTr && (index == 1 || index == 5 || index == 6))) {
	            	                // 텍스트 입력 필드를 생성하는 조건
...
<div id="layoutSidenav_content">
    <main>
        ...
        <h1 class="mt-4 dongle-title">&nbsp;&nbsp;강사 등록</h1>
        <hr>
					...
					<form name="searchFrm" class="search-form">
					    ...
					    <a href = "trainerManage.jsp" class = "refresh-btn">새로 고침</a>
                        ...
                        <div class="card">
          					  ...
          					  <thead>
       						     ...
       						     <tbody>
       						             <%
													Vector<UserBean> uvlist = uMgr.getAllTrainer(keyField, keyWord, start, cnt);
										...
										<%
											...
											for(int i = 0; i <numPerPage; i++){
												...
												UserBean uBean = uvlist.get(i);
   						             			TrainerBean tBean = uBean.getTrainerInfo();
											    ...
											    career = tBean.getCareer();
											    imgpath = tBean.getImgpath();
											    
											    if(tBean.getCareer()==null){
											    	career = "없음";
											    }
											    if(tBean.getImgpath()==null){
											    	imgpath = "없음";
											    }
