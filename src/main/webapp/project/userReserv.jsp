...
<!--  메인 사이드바 기본 html -->
 ...
 <%@ include file="adminSideBar.jsp" %>
							 ...
							 document.addEventListener('DOMContentLoaded', function() {
								    ...
								    editButtons.forEach(function(button){
								    	button.addEventListener('click', function(){
								    		...
								    		var rNumTd = tr.querySelector('td[data-id]')
								             var rnum = rNumTd.getAttribute('data-id');
								                
								                if(rnum) {
								                    rnum1 = rnum;
								                }
								    			
								    			var form = document.getElementById('rFinishForm');
								    			
								    			form.elements['rnum1'].value = rnum1;
					...
					<main>
						...
						<h1 class="mt-4  dongle-title">&nbsp;&nbsp;  상담 신청 내역 </h1>
						<hr>
                        <div class="card">
                        ...
                        <div class="card">
                                    ...
                                    <tbody>
     <%
													Vector<ReservationBean> pvlist = rMgr.getAllReserv();
										...
										<%
											...
											for(int i = 0; i <numPerPage; i++){
												...
												ReservationBean pBean = pvlist.get(i);
										        ...
										        if(pBean.getFnflag()==true){
										        	fnflag = "상담 완료";
										        }else if(pBean.getFnflag()==false){
										        	fnflag = "X";
										        }
										        
										        if(pBean.getCarflag()==true){
										        	carflag = "자차방문";
										        }else if(pBean.getCarflag()==false){
										        	carflag = "대중교통 이용";
										        }
										...
										<tr>
                                            ...
                                            <td><%=pBean.getName()%></td>
                                            <td><%=pBean.getPhonenum()%></td>
                                            <td><%=pBean.getEmail()%></td>
                                            <td><%=pBean.getPopulation()%></td>
                                            <td><%=pBean.getReservdate()%></td>
                                            <td><%=fr%></td>
                                            <td><%=carflag%></td>
                                            <td><%=ccflag%></td>
                                            <td><%=fnflag%></td>
                                            <td> <button class="editbtn" data-id="<%=i%>" >완료</button></td>
                    				     </tr>
                          			 <% }%>
                                  <% }%>  
...
<form id = "rFinishForm" name="rFinish" method="post" action="rFinishProc.jsp" >
    <input name="rnum1"  type="hidden">
</form>
