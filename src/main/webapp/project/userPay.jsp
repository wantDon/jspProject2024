...
<%
		...
		int numPerPage = 8; // 페이지당 레코드 개수 (5,10,20,30)
		...
		totalRecord = cMgr.getTotalPay(keyField, keyWord);
		
		//최근 한달
		totalRecord1 = cMgr.getTotalPay2(keyField, keyWord);
		totalprice = cMgr.getTotalPayPrice(keyField, keyWord);
		TotalCcflag = cMgr.getTotalCancel(keyField, keyWord);
		TotalPayBack = cMgr.getTotalPayBack(keyField, keyWord);
...
<%@include file = "adminSideBar.jsp" %>
					...
					<main>
						...
						<h1 class="mt-4  dongle-title">&nbsp;&nbsp;  회원 결제 내역 </h1>
				              ...
				              <hr>
									...
									<form name="searchFrm" class="search-form">
									    ...
									    <a href = "userPay.jsp" class = "refresh-btn">새로 고침</a>
                        ...
                        <div class="card">
          					  ...
          					  <thead>
       						     ...
       						     <tbody>
       						             <%
													Vector<PayItemsBean> pvlist = cMgr.getPayList(keyField, keyWord, start, cnt);
										...
										<%
											...
											for(int i = 0; i <numPerPage; i++){
										        ...
										        if(pBean.getPayback()==true){
										        	payback = "환불완료";
										        }else if(pBean.getPayback()==false){
										        	payback = "X";
										        }
										        
										        if(pBean.getCcflag()==true){
										        	ccflag = "취소완료";
										        }else if(pBean.getCcflag()==false){
										        	ccflag = "X";
										        }
										        
										%>	
										<tr>
                                            <td data-id=<%=pBean.getUsernum()%>><%=pBean.getUsernum()%></td>
                                            <td colspan = "2"> <%=pBean.getPaydate()%></td>
                                            <td><%=item%></td>
                                            <td><%=price%>원</td>
                                            <td><%=pBean.getPaymethod()%></td>
                                            <td><%=fr%></td>
                                            <td><%=payback%></td>
                                            <td><%=ccflag%></td>
                    				     </tr>
                                  ...
                                  <% }%>
                                  <tr>
									    <td colspan="4" ><hr style="border-color: red;"></td>
									    <td><font color="red"> 최근 한달&nbsp;---&nbsp;  판매수: <%=totalRecord1%>개&nbsp;//&nbsp;&nbsp;   판매액수: <%=totalprice1%>원 &nbsp;//&nbsp;&nbsp;   환불개수: <%=TotalPayBack%>개  &nbsp;//&nbsp;&nbsp;   판매액수: <%=TotalCcflag%>개</font></td>
									    <td colspan="4"><hr style="border-color: red;"></td>
									</tr>                           
